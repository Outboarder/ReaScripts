--[[
Description: Create Stereo Send from track under mouse to selected tracks (increment send channel)
Instructions:
Screenshot: 
Version: 1.0
Author: Outboarder
  ...
  reaper.Undo_BeginBlock()  
	...
	if S_track then
	  ...
	  for i = 0 ,Ct-1 do
		...
		reaper.SetMediaTrackInfo_Value( R_track, "I_NCHAN", R_ch )
		reaper.CreateTrackSend( S_track, R_track )
		reaper.BR_GetSetTrackSendInfo( S_track, 0, Last_send+i, "I_DSTCHAN", 1, R_ch )
	  end
	end

  reaper.Undo_EndBlock("Create Stereo Send from track under mouse to selected tracks", 0)
end  
